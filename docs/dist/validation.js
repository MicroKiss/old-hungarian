...
const SPACE = ' ';
const legalLatinCharacters = oldHungarianCharacters.map(({ latin }) => latin).join('') +
    oldHungarianCharacters.map(({ latin }) => capitalize(latin)).join('') +
    SPACE + LEGAL_DIGITS;
const legalLatinCharacterPattern = new RegExp(`^[${legalLatinCharacters}]+$`);
const legalOldHungarianCharacters = oldHungarianCharacters.map(({ small, large }) => small + large).join('') +
...
export function validateOldHungarianInput(text) {
    return legalOldHungarianCharacterPattern.test(text);
}
export function findIllegalOldHungarianCharacter(text) {
 ...
 * ```
 */
export function validateLatinInput(text) {
 ...
 * ```
 */
export function findIllegalLatinCharacter(text) {
